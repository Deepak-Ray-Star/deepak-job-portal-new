          ...
          <div className='flex gap-2 mb-6 mt-3'>
              {
                ...
                : ( userData?.resume &&
                  <div className='flex gap-2 '>
                      <a rel="noreferrer" target='_blank' href={userData.resume} className='bg-blue-100 text-blue-600 px-4 py-2 rounded-lg' >Resume</a>
                ...
                ? ( 
                    <tr>
                        <td colSpan="5" className="text-center py-4 text-gray-500">
                            No applications found.Apply to see..
                        </td>
                    </tr>
                ) 
                : (
                  userApplications?.map((job, index) => (
                      ...
                      <tr key={index}>
                        ...
                        <td className='py-1 px-4 border-b border-gray-200 max-sm:hidden'> {job.jobId.location}</td>
                        <td className='py-1 px-4 border-b border-gray-200 max-sm:hidden'> { moment(job.date).format('ll') } </td>
                        <td className='py-1 px-4 border-b border-gray-200 '>
                          <span className={` ${job.status==='Accepted'? 'bg-green-200': job.status === 'Rejected' ? 'bg-red-200':'bg-yellow-200'} px-4 py-1.5 rounded`}>{job.status}</span>
                        </td>
                      </tr>
                    ) 
                  )
